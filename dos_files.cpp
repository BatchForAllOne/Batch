...
bool DOS_MakeName(char const * const name, char * const fullname, Bit8u * drive)
{
    ...
    {
        /**
         * Ambos \0 e espaço são separadores e arquivo de relatório
         * de nomes de arquivo vazio não encontrado.
         */
    ...
    *drive = DOS_GetDefaultDrive();
    ...
    /**
     * Primeiro, vamos obter a unidade.
     */
    if (name_int[1] == ':')
    {
    ...
    /**
     * Agora analise o novo nome de arquivo para criar o
    ...
    while (!stop)
    {
        ...
        if ((upname[r] == '\\') || (upname[r] == 0))
        {
            ...
            if (ext)
            {
                ...
                {
                    /**
                     * Outro ponto na extensão => arquivo não encontrado. Ou o
                     * caminho não foi encontrado, dependendo se ainda estamos
                     * no estágio de verificação do diretório ou no estágio do
                     * arquivo.
                     */
                    if(stop)
                    {
...
{
    /**
     * Precisamos fazer o teste antes da remoção, pois não
     * podemos confiar no host para não autorizar a remoção
     * do diretório atual. Nunca mudamos de diretório. Tudo
     * acontece nas unidades.
     */
    Bit8u drive;
    ...
    /**
     * Veja se é o diretório atual.
     */
    char currdir[DOS_PATHLENGTH] =
    {
        0
    };
    ...
    /**
     * Falhou: Sabemos que existe e não é o diretório atual,
     * concluir que não está vazio.
     */
    DOS_SetError(DOSERR_ACCESS_DENIED);
...
bool DOS_Rename(char const * const oldname, char const * const newname)
{
    ...
    /**
     * Teste se o destino existe => sem acesso.
     */
    Bit16u attr;
    ...
    /**
     * A fonte deve existir, verifique o caminho ?
     */
    ...
    /**
     * Se ainda assim falhar. Qual falha devemos dar ? CAMINHO
     * NÃO ENCONTRADO ou EACESS.
     */
...
bool DOS_FindFirst(char * search, Bit16u attr, bool fcb_findfirst)
{
    ...
    {
        /**
         * Instalador Dark Forces, mas c:\ é adequado para rótulos
         * de volume (definido exclusivamente).
         */
    ...
    /**
     * Verifique os dispositivos. FindDevice também verifica
     * o subdiretório principal.
     */
    bool device = (DOS_FindDevice(search) != DOS_DEVICES);

    /**
     * Divida a pesquisa em diretórios e padrão.
     */
    char * find_last;
    ...
    if (device)
    {
        ...
        /**
         * Questão: usar data e hora atual.
         */
...
bool DOS_FindNext(void)
{
    ...
    if(i >= DOS_DRIVES || !Drives[i])
    {
         ...
         */
        LOG(LOG_FILES, LOG_ERROR)("Specialist search!!!!");
        DOS_SetError(DOSERR_NO_MORE_FILES);
...
{
    /**
     * A criação de um dispositivo é o mesmo que abri-lo
     * no instalador Tc201.
     *
     */
    ...
    /**
     * Verifique se há um identificador de arquivo
     * livre.
     */
    ...
    /**
     * Temos uma posição na tabela principal agora,
     * encontre uma na tabela psp.
     */
    *entry = psp.FindFreeFileEntry();
    ...
    /**
     * Não permitir que diretórios sejam criados.
     */
...
{
    /**
     * Primeiro verifique se há dispositivos.
     */
    if (flags > 2)
    {
        LOG(LOG_FILES, LOG_ERROR)("Special file open command %X file %s", flags, name);
    } else
    {
    ...
    {
        /**
         * NÃO PERMITA que os diretórios sejam abertos. (pule o
         * teste se o arquivo for um dispositivo).
         */
    ...
    /**
     * Primeiro verifique se o nome está correto.
     */
    if (!DOS_MakeName(name, fullname, &drive))
    {
        return false;
    ...
    /**
     * Verifique se há um identificador de arquivo livre.
     */
    for (i = 0; i < DOS_FILES; i++)
    {
        if (!Files[i])
    ...
    /**
     * Temos uma posição na tabela principal agora encontre
     * uma na tabela psp.
     */
    *entry = psp.FindFreeFileEntry();
    ...
    {
        /**
         * Teste se o arquivo existe, mas aberto no modo de
         * leitura/gravação (e protegido contra gravação).
         */
...
{
    /**
     * Questão: Ainda sem suporte: Bit 13 de bandeiras (int
     * 0x24 em uma falha).
     */
    Bit16u result = 0;
    ...
    } else
    {
        ...
        {
            /**
             * Usa código de falha da falha ao abrir.
             */
            return false;
        }
        ...
        {
            /**
             * Usa código de falha da falha na criação.
             */
            return false;
        }
 ...
 * Questão: Bit16u attr.
 *
 * Esta função não altera os atributos do arquivo apenas faz
 * alguns testes se o arquivo estiver disponível retorna
 * false ao usar em cdrom (stonekeep).
 */
bool DOS_SetFileAttr(char const * const name, Bit16u)
{
    Bit16u attrTemp;
...
{
    /**
     * Questão: Adicione um melhor suporte para dispositivos.
     */
    Bit8u drive;
...
{
    /**
     * Não duplique os manipuladores do console.
     *
     * if (entry <= STDPRN)
     * {
     *     *newentry = entry;
...
bool DOS_CreateTempFile(char * const name, Bit16u * entry)
{
    ...
    {
        /**
         * Arquivo temporário criado no diretório raiz.
         */
        tempname[0] = '\\';
        tempname++;
    } else
    ...
    /**
     * Adicione lixo aleatório ao final do nome e tente abrir.
     */
    do
    {
        Bit32u i;
...
Bit8u FCB_Parsename(Bit16u seg, Bit16u offset, Bit8u parser, char *string, Bit8u *change)
{
    ...
    {
        /**
         * Unidade padrão forçada, isso invalida intencionalmente
         * um FCB estendido.
         */
    ...
    /**
     * Primeiro pegue os dados antigos do fcb.
     */
    #ifdef _MSC_VER
        /**
         *
    ...
    /**
     * Obtenha as informações antigas do fcb anterior.
     */
    ...
    /**
     * Faixa do principal sepetaror.
     */
    if((parser & PARSE_SEP_STOP) && *string)
    {
        /**
         * Ignorar separador inicial.
         */
        char sep[] = FCB_SEP;
        char a[2];
...
static void SaveFindResult(DOS_FCB & find_fcb)
{
    ...
    /**
     * Crie um arquivo e extensão corretas.
     */
    ...
    /**
     * Apenas adiciona atributo se fcb for estendido.
     */
    fcb.SetAttr(attr);
...
bool DOS_FCBFindFirst(Bit16u seg, Bit16u offset)
{
    ...
    /**
     * Obtém atributos de pesquisa se estendido.
     */
    fcb.GetAttr(attr);
...
Bit8u DOS_FCBRead(Bit16u seg, Bit16u offset, Bit16u recno)
{
    ...
    if (!DOS_ReadFile(fhandle, dos_copybuf, &toread))
    {
        return FCB_READ_NODATA;
    }
    ...
    /**
     * Copybuffer de almofada zero para rec_size.
     */
    if (toread < rec_size)
    {
        Bitu i = toread;
...
Bit8u DOS_FCBWrite(Bit16u seg, Bit16u offset, Bit16u recno)
{
    ...
    /**
     * O tempo não acompanha o fim do dia.
     */
...
{
    /**
     * Se a restauração for verdadeira: leitura aleatória, caso
     * contrário, leitura aleatória do bloco. A leitura aleatória
     * atualiza o bloco antigo e o registro antigo para refletir
     * os dados aleatórios antes da leitura !. Os dados aleatórios
     * não são atualizados. (o usuário deve fazer isso). A
     * leitura aleatória de blocos atualiza esses campos para
     * refletir o estado após a leitura.
     */

    /**
     * Questão: numRec deve retornar a quantidade de registros lidos.
     * Ainda não implementado, pois não tenho certeza de como contar
     * com os estados de falha (Parcial/Com falha).
     */
    ...
    /**
     * Defina o registro correto a partir dos dados aleatórios.
     */
    ...
    {
        /**
         * Armazene isso para depois da leitura.
         */
    ...
    /**
     * Atualize o ponteiro do registro aleatório com a nova
     * posição somente quando a restauração for falsa.
     */
    if(!restore)
    {
...
Bit8u DOS_FCBRandomWrite(Bit16u seg, Bit16u offset, Bit16u numRec, bool restore)
{
    ...
    if (numRec > 0)
    {
        ...
        {
            /**
             * dos_fcbwrite devolve 0 (falso), quando verdadeiro.
             */
            error = DOS_FCBWrite(seg, offset, (Bit16u)i);
...
{
    /**
     * FCB DELETE respeita as cartas do jogo. ele retornará
     * verdadeiro se um ou mais arquivos forem excluídos. Para
     * obter isso: o dta é definido como dta temporário no qual
     * os arquivos encontrados são armazenados. Este não pode
     * ser o tempdta, pois é usado por fcbfindfirst.
     */
    RealPt old_dta = dos.dta();
    ...
    while(nextfile)
    {
        ...
        {
            /**
             * Pelo menos um arquivo deletado.
             */
            return_value = true;
        }
...
{
    /**
     * Configurar as alças de arquivo.
     */
    Bit32u i;
    ...
    /**
     * Configurar o sistema de disco virtual.
     */
